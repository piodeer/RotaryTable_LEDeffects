...
int motorSpeed = 500;     // Schritte/Sek (intern)
...
<meta name="fw-version" content="1.0">
...
void loop(){
  ...
  // Gemeinsamer Loop-Takt für alle Modi (ca. 250 Hz bei 4 ms)
  unsigned long now = millis();
  if (now - lastLoopTick < 4) {
    return;
  }
  lastLoopTick = now;

  updateMotorRamping();
  // Motor immer gleichmäßig takten, unabhängig von LED-Animation
  for (int i = 0; i < 5; i++) {
    stepper.runSpeed();
  }
